...
#Function to get all signals' indexes and names when input location_name   
...
def get_det_config_in_one(location_name,conn_string):
    conn = connect_db(conn_string)
    location_id = get_location_id(location_name, conn_string)
    cursor = conn.cursor('cursor_unique_name', cursor_factory = psycopg2.extras.DictCursor)
    cursor.execute("SELECT idx, sgidx,name FROM controller_config_det WHERE fk_cid =" + str(location_id))
    rows = cursor.fetchall()
    detectors = []
    for i in range(len(rows)):
        detectors.append(rows[i])
...
def signalState(sgStateCode):   
    green_state_list = ["0","1","3","4","5","6","7","8",":"]
    amber_state_list=[";","<","=",">","I"]
    red_state_list=["?","@","A","B","C","D","E","F","G","H","J"]
    Unknown_state ="."
